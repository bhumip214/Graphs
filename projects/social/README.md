...
1. To create 100 users with an average of 10 friends each, how many times would you need to call `addFriendship()`? Why?
   ==> When we call `addFriendship()`, it adds two friendships as the connection between two users is bi-directional. Therefore, we can use `avgFriendships * numUsers // 2` formula, i.e. 10 \* 100 // 2 = 500. Hence, `addFriendship()` will be called 500 times.
